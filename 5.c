...
void input_from_user(struct Process processes[], int num_processes) {
    ...
    //taking input for 1st process where arrival time can be 0
    do{
        processes[0].pid = 1;
        printf("Enter arrival time for process P1: ");
        scanf("%d", &processes[0].arrival_time);
        if (processes[0].arrival_time < 0) {
            printf("Error: Burst time must be greater than or equal to 0 for process P1. Please re-enter.\n");
        }
    }while(processes[0].arrival_time < 0);

    do {
        printf("Enter burst time for process P1: ");
        scanf("%d", &processes[0].burst_time);

        if (processes[0].burst_time <= 0) {
            printf("Error: Burst time must be greater than 0 for process P1. Please re-enter.\n");
        }
    } while (processes[0].burst_time <= 0);
    processes[0].completion_time = -1;
