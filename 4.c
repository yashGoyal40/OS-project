...
//for performing quick sort with arrival time in order to improve efficiency  
int partitionWithArrivalTime(struct Process processes[], int low, int high);
void quickSortWithArrivalTime(struct Process processes[], int low, int high);
void sort_by_arival_time(struct Process processes[], int num_processes);

//for performing quick sort with burst time in order to improve efficiency 
int partitionWithBurstTime(struct Process processes[], int low, int high);
void quickSortWithBurstTime(struct Process processes[], int low, int high);
...
// partitioning is done in order to impliment the quick sort
int partitionWithArrivalTime(struct Process processes[], int low, int high) {
    int pivot = processes[high].arrival_time; // You can change this to burst_time for sorting by burst time
    int i = (low - 1);

    for (int j = low; j <= high - 1; j++) {
        if (processes[j].arrival_time < pivot) { // Change this condition for sorting by burst time
            i++;
    ...
    // Swap processes[i + 1] and processes[high]
...
//quick sort in order to increase the efficiency
void quickSortWithArrivalTime(struct Process processes[], int low, int high) {
    if (low < high) {
        // Get the partition index
        int pi = partitionWithArrivalTime(processes, low, high);

        // Sort the elements before and after partition
        quickSortWithArrivalTime(processes, low, pi - 1);
        quickSortWithArrivalTime(processes, pi + 1, high);
    }
}

//will sort the array of processes with respect to arival time
void sort_by_arival_time(struct Process processes[], int num_processes) {
    quickSortWithArrivalTime(processes, 0, num_processes - 1);
}


// partitioning is done in order to impliment the quick sort
int partitionWithBurstTime(struct Process processes[], int low, int high) {
    int pivot = processes[high].burst_time; // You can change this to burst_time for sorting by burst time
    int i = (low - 1);

    for (int j = low; j <= high - 1; j++) {
        if (processes[j].burst_time < pivot) { // Change this condition for sorting by burst time
            i++;
            // Swap processes[i] and processes[j]
            struct Process temp = processes[i];
            processes[i] = processes[j];
...
void quickSortWithBurstTime(struct Process processes[], int low, int high) {
    if (low < high) {
        // Get the partition index
        int pi = partitionWithBurstTime(processes, low, high);

        // Sort the elements before and after partition
        quickSortWithBurstTime(processes, low, pi - 1);
        quickSortWithBurstTime(processes, pi + 1, high);
    }
}


//will sort the array of processes with respect to burst time
void sort_by_burst_time(struct Process processes[], int num_processes) {
    quickSortWithBurstTime(processes, 0, num_processes - 1);
}
